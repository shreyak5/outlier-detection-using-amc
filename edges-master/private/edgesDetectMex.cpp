...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  const mwSize gtWidth = (int) mxGetScalar(mxGetField(opts,0,"gtWidth"));
  ...
  const mwSize nTreesEval = (int) mxGetScalar(mxGetField(opts,0,"nTreesEval"));
  ...
  const mwSize h1 = (int) ceil(double(h-imWidth)/stride);
  const mwSize w1 = (int) ceil(double(w-imWidth)/stride);
  const mwSize h2 = h1*stride+gtWidth;
  const mwSize w2 = w1*stride+gtWidth;
  const int imgDims[3] = {h,w,Z};
  const int chnDims[3] = {h/shrink,w/shrink,nChns};
  const mwSize indDims[3] = {h1,w1,nTreesEval};
  const mwSize outDims[3] = {h2,w2,1};
  const mwSize segDims[5] = {gtWidth,gtWidth,h1,w1,nTreesEval};
